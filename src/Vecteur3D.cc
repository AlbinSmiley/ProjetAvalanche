...
Vecteur3D operator*(double lambda,
                    Vecteur3D vec) { // on copie vec car on ne le modifie pas
  return vec *= lambda;
}

Vecteur3D operator*(
    Vecteur3D vec,
    double lambda) { // on ajoute cette méthode pour plus de féxiblité dans la
                     // multiplication par un scalaire (vec*lambda = lambda*vec)
  return lambda * vec;
}

Vecteur3D operator/(double lambda, Vecteur3D vec) { return vec /= lambda; }

Vecteur3D operator/(Vecteur3D vec, double lambda) { return lambda / vec; }
