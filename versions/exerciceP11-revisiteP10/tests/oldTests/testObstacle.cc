...
int main() {
  ...
  for (auto const &plan : plans) { // on teste chauqe point sur ce plan
    cout << "Plan d'origine " << plan.getPosition() << " et de normale "
         << plan.getNormale() << " :" << endl;

    for (auto const &point : points) {
      cout << "distance à " << point << " : " << plan.PointPlusProche(point)
           << endl; // on claule la projection orthogonale du point sur le plan
    }
  }
