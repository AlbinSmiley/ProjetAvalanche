...
public:
  ...
  // modificateur
  Vecteur3D &operator+=(Vecteur3D const &); // retour de reference d'un vecteur
  // pour pouvoir définir + par +=
...
Vecteur3D operator+(Vecteur3D,
                    Vecteur3D const &); // on ne met pas de const devant le type
                                        // de retour car cela ne
// nous semble pas très utile, nous n'allons pas faire des
// trucs trop "bizarre" avec les oprations ... par exemple on
// ne feras pas de truc du genre : (vec1+vec2) = vec3 ...
