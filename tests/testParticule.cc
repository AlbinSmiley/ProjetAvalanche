...
using EnsembleParticule = vector<Particule>;

void indent(int n) { cout << setw(n) << ""; }

void afficheParticuleEtat(size_t num_particule, EnsembleParticule sys) {
  ...
  for (size_t i = 0; i < sys.size(); i++) {
    if (i == num_particule) {
      ...
      indent(6);
      cout << "Particule " << i + 1 << " : " << endl;
      indent(12);
      cout << "distance entre grains = " << distance(main_part, sys[i]) << endl;
      indent(12);
      cout << "forceLJ = " << main_part.forceLJ(sys[i]) << endl;
      indent(12);
      cout << "df = " << main_part.ajouteForce(sys[i])
  ...
  cout << "dx = " << dx << endl;
  indent(12);
  cout << "v  = " << main_part.get_vitesse() << endl;
  indent(12);
  cout << "x  = " << main_part.get_position() << endl;
}
...
int main() {
  ...
  EnsembleParticule sys = {p1, p2, p3};
  ...
  indent(6);
  cout << "Particule 3 : " << p3 << endl << endl << setw(0);
